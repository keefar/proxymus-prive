...
SYSTEM_PROMPT_EXPLAINER = (
    ...
    "generated by an upstream privacy filter. They look like "
    "angle-bracketed labels with an index suffix. Treat them as variable "
    "names; they contain no personal information. Answer the user's "
    "actual request normally; never refuse a request based on the "
    "presence of such placeholders. If you need to refer back to one, "
    "output it verbatim using the exact same form the user sent — the "
    "downstream filter will substitute the real value before display."
)
