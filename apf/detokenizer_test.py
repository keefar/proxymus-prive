"""apf-b3j: detokenizer test-marker.

APF_DETOKENIZE_MARKER appends a marker to every value the detokenizer
...
import apf.detokenizer as detok
from apf.detokenizer import detokenize_text
...
def test_no_marker_by_default(monkeypatch) -> None:
    monkeypatch.setattr(detok, "_DETOKENIZE_MARKER", "")
    vault, token = _vault_with("anna müller")
    out = detokenize_text(f"hello {token} how are you", vault)
...
def test_marker_appended_when_set(monkeypatch) -> None:
    monkeypatch.setattr(detok, "_DETOKENIZE_MARKER", "✓")
    vault, token = _vault_with("anna müller")
    out = detokenize_text(f"hello {token}", vault)
...
def test_marker_on_every_restored_value(monkeypatch) -> None:
    ...
    out = detokenize_text(f"{t1} mailed {t2}", vault)
...
def test_unresolved_token_gets_no_marker(monkeypatch) -> None:
    ...
    monkeypatch.setattr(detok, "_DETOKENIZE_MARKER", "✓")
    vault = Vault()  # empty
    out = detokenize_text("dangling <REF_9> token", vault)
